...
namespace kiv::hal::gpio {
    ...
    class GPIO_Bank {
        ...
        std::array<GPIO_MODE, NUMBER_OF_PINS_IN_THE_BANK> cached_modes{};
        std::array<GPIO_OUTPUT_TYPE, NUMBER_OF_PINS_IN_THE_BANK> cached_output_type{};
        std::array<GPIO_OUTPUT_SPEED, NUMBER_OF_PINS_IN_THE_BANK> cached_output_speed{};


        uint32_t cached_pull_up_down_register{};

        uint32_t cached_alternate_function_low_register{};
        uint32_t cached_alternate_function_high_register{};
        ...
        void set_register(uint32_t value) {
            ...
            if constexpr (EMULATION) {
                std::printf("set_register %p : 0x%08x\n", ptr, value);
            }
        }
    ...
    public:
        ...
        void set_pin() {
            set_register<GPIO_HW_Registers::BITSET_RESET>(get_pin_mask(GPIO_PIN));
        }

        template<GPIO_PIN GPIO_PIN>
        void reset_pin() {
            set_register<GPIO_HW_Registers::BITSET_RESET>(get_pin_mask(GPIO_PIN) << 16u);
        }
    };
}
